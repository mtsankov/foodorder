...
<?php
$uid=$_SESSION['fosuid'];
      $query=mysqli_query($con,"select * from  tblorderaddresses  where UserId='$uid'  ORDER BY ID DESC");
      $count=1;
              while($row=mysqli_fetch_array($query))
              { ?>                  
                                <div class="row">
         ...
         <h5><a href="order-details.php?orderid=<?php echo $row['Ordernumber'];?>">Поръчка # <?php echo $row['Ordernumber'];?></a></h5> 
         <p><b>Дата: </b> <?php echo $row['OrderTime']?></p>
                                            <ul class="list-inline">
                                                <li class="list-inline-item"><i class="fa fa-check"></i> 
                                                    <?php $status=$row['OrderFinalStatus'];
if($status==''){
 echo "Изчакване на потвърждаване";   
...
?>
 <a href="javascript:void(0);" onClick="popUpWindow('trackorder.php?oid=<?php echo htmlentities($row['Ordernumber']);?>');" title="Track order">Проследяване на поръчка</a></li>
                                            </ul>
                                        </div>
                                    ...
                                    <div class="col-sm-12 col-md-12 col-lg-4 text-xs-center">
                                     ...
                                     <a href="order-details.php?orderid=<?php echo $row['Ordernumber'];?>" class="btn theme-btn-dash">Детайли</a> </div>
                                        </div>
                                        <!-- end:right info -->
                                    </div>
                                </div>
